...
def generate_ai_comment(student_name, biocode, constitution, height_gap, lag_cause, sports):
    """
    Calls local Ollama to write an in-depth, rich premium coach comment for the report.
    Falls back gracefully to a highly detailed, professional rule-based comment if Ollama is offline.
    """
    ...
    fallback_comment = f"### [1. 피지컬 성장 분석]\n" \
                       ...
                       f"### [2. 영양 및 소화 흡수 전략]\n" \
                       f"아이의 기질인 '{constitution}' 특성에 최적화된 맞춤형 장벽 보호 및 흡수율 극대화 식이 전략이 핵심입니다. {const_advice} 장내 흡수 통로가 잘 열릴 수 있도록 식습관 전반을 가이드라인에 맞춰 보완해 주시기 바랍니다.\n\n" \
                       f"### [3. 맞춤형 운동 및 수면 설계]\n" \
                       f"{sports} 종목에 최적화된 하체 뼈 길이 및 상체 wingspan 비율을 극대화하려면 맞춤 자극 요법이 동반되어야 합니다. 특히 성장의 저해 요인인 '{lag_cause}'을(를) 해결하기 위해 아래 처방을 실천에 즉각 적용하십시오:\n" \
                       f"{cause_solution}\n" \
    ...
    prompt = f"당신은 엘리트 유소년 스포츠 피지컬 성장 전문가 그룹인 '피지컬업 333 피지컬 코칭 팀'입니다.\n" \
             f"다음 상세 정보를 바탕으로 {student_name} 학생의 부모님이 깊이 신뢰하고 즉각 실천할 수 있는 " \
             f"초프리미엄 피지컬 성장 분석 및 맞춤형 트레이닝/영양 가이드를 대단히 상세하고 깊이 있게 한국어로 작성해 주세요.\n\n" \
             f"정보:\n" \
             f"- 아동 이름: {student_name}\n" \
             f"- 희망/진행 운동 종목: {sports}\n" \
             f"- 성장 등급 코드 (BioCode): {biocode} (선천-대사-생활 등급 순서)\n" \
             f"- 한의학적 사상 체질: {constitution}\n" \
             f"- 유전 예상 신장 대비 현재 실측 신장 격차: {height_gap:.1f} cm (음수면 성장이 뒤처진 상태, 양수면 우수)\n" \
             f"- 성장의 주된 저해(제한) 원인: {lag_cause}\n\n" \
             f"작성 가이드라인:\n" \
             f"1. 글의 길이와 디테일은 제공된 아동의 성장에 필요한 핵심 처방에 맞게 피지컬 코칭 팀의 관점에서 최대한 풍성하고 전문적으로 작성하세요. '젬마', '잼마' 등 특정 개인의 이름이나 페르소나를 절대 언급하지 말고, 오직 '피지컬업 333 피지컬 코칭 팀'의 관점에서만 기술하십시오.\n" \
             f"2. 의료법 저촉을 방지하기 위해 '의학', '의사', '의학적 진단', '처방', '치료', '소견', '의료' 등 병원이나 의료 행위로 오인될 수 있는 단어는 단 하나도 사용하지 마십시오.\n" \
             f"3. 텍스트의 타이틀과 전체 구조는 반드시 다음 마크다운 단락 구성을 100% 철저히 지켜 주십시오. 다른 형태의 기호나 머리말은 절대 쓰지 마세요:\n" \
             f"   - ### [1. 피지컬 성장 분석] : BioCode({biocode}) 분석을 통해 본 아동의 현 대사 상태와 유전키 격차({height_gap:.1f}cm) 원인을 분석합니다. 본문에서 반드시 3장의 예상키 대비 실측 격차 수치({height_gap:.1f}cm) 및 BioCode 점수를 인용하며 설명하세요.\n" \
             f"   - ### [2. 영양 및 소화 흡수 전략] : 체질({constitution})과 성장의 주된 저해 원인({lag_cause})을 고려한 흡수율 개선 식단 및 영양 보충 가이드입니다. 본문에서 5대 기질 오행 분포를 짚으며 어떤 영양소 흡수가 취약한지 분석하세요.\n" \
             f"   - ### [3. 맞춤형 운동 및 수면 설계] : {sports} 종목 특성에 어울리는 뼈 성장 자극 운동 요법 및 성장호르몬 분비 촉진을 위한 핵심 수면 타이밍입니다. 본문에서 부위별 근육 발달 수준이나 종목 특징을 함께 분석해 수면 및 운동 팁을 기술하세요.\n" \
             f"4. 학부모가 읽기 편하도록 따뜻하면서도 과학적 권위가 있는 '~입니다', '~합니다' 톤을 유지하세요.\n" \
             f"5. 불필요한 백틱(```), JSON 기호, 마크다운 특수문자 없이 자연스러운 텍스트로만 출력하세요."
...
def ai_double_check_comment(student_name, biocode, constitution, height, weight, muscle, fat, original_comment):
    ...
    prompt = f"당신은 엘리트 유소년 피지컬 성장 분석 리포트의 수석 최종 검수관인 '피지컬업 333 피지컬 코칭 팀'입니다.\n" \
             ...
             f"3. **의료법 저촉 방지**: 가이드 내부에 '의학', '의사', '의학적 진단', '소견', '처방', '치료', '의료' 등 병원/의료 오해 소지가 있는 표현이나 '젬마', '잼마' 등 특정 인물 이름이 있다면 이를 '피지컬업 333 피지컬 코칭 팀'의 피지컬 가이드 가이드로 정교하게 보정하십시오.\n" \
             f"4. **헤더 규격 및 단락 구조 유지**: 원본 가이드의 3대 단락 구조인 '### [1. 피지컬 성장 분석]', '### [2. 영양 및 소화 흡수 전략]', '### [3. 맞춤형 운동 및 수면 설계]' 헤더 구조는 반드시 100% 동일하게 지켜 주십시오.\n" \
