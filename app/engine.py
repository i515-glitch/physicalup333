...
def calculate_biocode(input_data):
    ...
    # Reverse reverse-scored questions: 6 - score
    rev_questions = [18, 20, 21, 23, 27, 30, 32, 33, 34, 37, 39]
    q = {}
    for q_idx in range(16, 40):
        val = raw_q.get(q_idx, 3)
        if q_idx in rev_questions:
            q[q_idx] = 6 - val
        else:
            q[q_idx] = val
    ...
    # Based on original responses to sensitive/anxious/weak questions (Q18, Q20, Q21, Q23, Q27, Q30)
    ...
    # Higher average original score means higher anxiety/sensitivity, lowering the score.
    ...
    # 2-2. 인바디 대사 (17.5점 만점)
    # SMM (Skeletal Muscle Mass) ratio to Body Weight
    muscle_ratio = (muscle / weight) * 100.0
    # Score SMM ratio
    if muscle_ratio >= 48.0:
        muscle_score = 100.0
    elif muscle_ratio <= 35.0:
        muscle_score = 30.0
    else:
        muscle_score = 30.0 + ((muscle_ratio - 35.0) / 13.0) * 70.0
        
    # Fat ratio score (Optimal: 12% - 16%)
    if 12.0 <= fat <= 16.0:
        fat_score = 100.0
    elif fat < 12.0:
        fat_score = max(50.0, 100.0 - (12.0 - fat) * 10.0) # slightly penalize underweight fat
    else:
        fat_score = max(20.0, 100.0 - (fat - 16.0) * 8.0) # penalize high body fat
        
    # BMI score (Optimal: 18.5 - 23.0)
    bmi = weight / ((height / 100.0) ** 2)
    if 18.5 <= bmi <= 23.0:
        bmi_score = 100.0
    elif bmi < 18.5:
        bmi_score = max(40.0, 100.0 - (18.5 - bmi) * 15.0)
    else:
        bmi_score = max(30.0, 100.0 - (bmi - 23.0) * 10.0)
    ...
    # Cutoff thresholds
    # Body (40 max): 0-14=1, 14-28=2, 28-40=3
    if body_total <= 14.0:
        body_grade = 1
    ...
    # Metabolism (35 max): 0-11=1, 11-24=2, 24-35=3
    if metab_total <= 11.0:
        metab_grade = 1
    ...
    # Behavior (25 max): 0-8=1, 8-17=2, 17-25=3
